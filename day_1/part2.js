...
const getNum = (str) => {
    ...
    // case when we have only one digit
    if (digits.length === 1) {
        const digit = `${digits[0]}`;
        if (digit.length > 1) {
            return parseInt(`${digit[0]}${digit[digit.length - 1]}`, 10);
        }
...
const replaceIntDigits = (str) => {
    ...
    let res = str;
    for (const [key, value] of Object.entries(strNumbersMap)) {
    ...
    for (let i = 0; i < res.length; i++) {
        const tmpStr = res.substring(0, i);
        if (!firstStr) {
            for (const [key, value] of Object.entries(strNumbersMap)) {
    ...
    for (let i = res.length - 1; i >= 0; i--) {
        const tmpStr = res.substring(i, res.length);
        if (!lastStr) {
            for (const [key, value] of Object.entries(strNumbersMap)) {
                if (tmpStr.includes(key)) {
                    lastStr = key;
    ...
    return `${strNumbersMap[firstStr]}${strNumbersMap[lastStr]}`;
}

let res = 0;
for (const el of arr) {
    const num = getNum(replaceIntDigits(el));
    res += num;
}
